...
+ (NSNumber *)sy_isImageFileValidAtURL:(NSURL *)url
{
    ...
    NSNumber *expectedFileSize = nil;
    
    NSDictionary *options = @{(NSString *)kCGImageSourceShouldCache:@(NO)};
    CFDictionaryRef imageProperties = CGImageSourceCopyPropertiesAtIndex(imageSource, 0, (CFDictionaryRef)options);
    if (imageProperties) {
        expectedFileSize = (NSNumber *)CFDictionaryGetValue(imageProperties, kCGImagePropertyFileSize);
        CFRelease(imageProperties);
    }
    ...
    NSNumber *fileSize = [[NSFileManager defaultManager] attributesOfItemAtPath:url.path error:NULL][NSFileSize];
    return @([expectedFileSize isEqualToNumber:fileSize]);
}

+ (UIImage *)sy_validImageWithContentsOfFile:(NSString *)path
{
    NSNumber *imageValid = [self sy_isImageFileValidAtURL:[NSURL fileURLWithPath:path]];
    if (imageValid && !imageValid.boolValue)
        return nil;
